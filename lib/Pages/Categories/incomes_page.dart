...
class IncomesPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    List<String> incomeKeys = new List();

    for (var key in ItemModel.catMap.keys) {
      if (ItemModel.catMap[key].type == ItemType.income) {
        incomeKeys.add(key);
      }
    }
    ...
    return GridView.count(
      ...
      mainAxisSpacing: 7,
      crossAxisCount: 4,
      children: List.generate(incomeKeys.length + 1, (index) {
        if (index >= incomeKeys.length) {
          return Container(
        ...
        return CategoryBtn(
            category: ItemModel.catMap[incomeKeys[index]],
  ...
  void _openAddCategoryPage(context) {
    Navigator.push(
      context,
      MaterialPageRoute(
          builder: (context) => AddNewCategoryPage(ItemType.income)),
    );
  }
}
